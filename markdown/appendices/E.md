...
| 17 | --cpr-reuse-setup | A positive integer that defines if the [CPR](#__RefHeading___Toc27871_3671211675) solver should re-use the AMG setup. Valid options are: Changed the default value from three to four. | 4 |
...
| 31 | --enable-dry-run | A boolean value that specifies if the simulation should actually run (true), or just check the input deck (false). This option is equivalent to activating the [NOSIM](#__RefHeading___Toc251745_4250154414) keyword in the [RUNSPEC](#__RefHeading___Toc55591_1778172979) section of the input deck (see section [Error: Reference source not found Error: Reference source not found](#5.2.32.NOSIM – Activates the No Simulation Mode for Data File Checking\|outline)). The default value of “auto’ will use whatever is stipulated in the input deck via the [NOSIM](#__RefHeading___Toc251745_4250154414) keyword. | “auto” |
...
| 33 | --enable-esmry | A boolean value that switches on (true) or off (false) the output of [SUMMARY](#__RefHeading___Toc43949_784232322) vectors to the ESMRY file for fast loading of summary data. | false |
...
| 42 | --enable-tuning | A boolean value that instructs OPM Flow to read the time stepping and convergence parameters from the [TUNING](#__RefHeading___Toc146744_4203985108) keyword in the input deck, if set to true. Note that only some of the [TUNING](#__RefHeading___Toc146744_4203985108) keyword parameters are processed. | false |
...
| 160 | --sched-restart | A boolean value that determines for a restart run if the case should initialize wells and groups from the historical [SCHEDULE](#__RefHeading___Toc43945_784232322) section (true), or from the well and group data on the restart file (false). Note that the commercial simulator always uses data from the restart file (false). | false |
...
| 14 | --cpr-reuse-setup | A positive integer that defines if the [CPR](#__RefHeading___Toc27871_3671211675) solver should re-use the AMG setup. Valid options are: Changed the default value from three to four. | 4 |
...
| 30 | --enable-esmry | A boolean value that switches on (true) or off (false) the output of [SUMMARY](#__RefHeading___Toc43949_784232322) vectors to the ESMRY file for fast loading of summary data. | false |
...
| 39 | --enable-tuning | A boolean value that instructs OPM Flow to read the time stepping parameters from the [Error: Reference source not found](#11.3.48.TUNING - Numerical Tuning Control\|outline) keyword in the input deck, if set to true. Note that only the first record of the [TUNING](#__RefHeading___Toc146744_4203985108) keyword is processed. | false |
...
| 166 | --time-step-control-file-name | A character string enclosed in quotes that specifies a filename where time steps are specified. The default is the character string timesteps. For instance time steps can be generated by the ecl_summary application in libecl as per the following UNIX command line: path_to_libecl_applications/ecl_summary DECK TIME > filename Where: DECK is the name of the data deck you want to get the time steps from, [TIME](#__RefHeading___Toc1252966_4250154414) tells the application to return the timing for the run, and "filename" is the name of the file the times are piped to. | “timesteps” |
...
| 9 | --cpr-reuse-setup | A positive integer that defines if the [CPR](#__RefHeading___Toc27871_3671211675) solver should re-use the AMG setup. Valid options are: Changed the default value from three to four. | 4 |
...
| 17 | --ecl-enable-tuning | Honor some aspects of the [TUNING](#__RefHeading___Toc146744_4203985108) keyword from the ECL deck. | false |
...
| 31 | --enable-esmry | A boolean value that switches on (true) or off (false) the output of [SUMMARY](#__RefHeading___Toc43949_784232322) vectors to the ESMRY file for fast loading of summary data. | false |
...
| 40 | --enable-tuning | A boolean value that instructs OPM Flow to read the time stepping parameters from the [Error: Reference source not found](#11.3.48.TUNING - Numerical Tuning Control\|outline) keyword in the input deck, if set to true. Note that only the first record of the [TUNING](#__RefHeading___Toc146744_4203985108) keyword is processed. | false |
...
| 16 | --ecl-enable-tuning | Honor some aspects of the [TUNING](#__RefHeading___Toc146744_4203985108) keyword from the ECL deck. | false |
...
| 28 | --enable-dry-run | A boolean value that specifies if the simulation should actually run, (true) or just check the input deck (false). This option is equivalent to activating the [NOSIM](#__RefHeading___Toc251745_4250154414) keyword in the [RUNSPEC](#__RefHeading___Toc55591_1778172979) section of the input deck (see section [Error: Reference source not found Error: Reference source not found](#5.2.32.NOSIM – Activates the No Simulation Mode for Data File Checking\|outline)). The default value of “auto’ will use whatever is stipulated in the input deck via the [NOSIM](#__RefHeading___Toc251745_4250154414) keyword. | “auto” |
| 29 | --enable-ecl-output | A boolean value that specifies if the binary output files (restart,  summary files, etc.)  should be written in the commercial simulator’s format (true), or OPM Flow’s format (false). | true |
| 30 | --enable-esmry | A boolean value that switches on (true) or off (false) the output [SUMMARY](#__RefHeading___Toc43949_784232322) vectors to  the ESMRY file for fast loading of summary data. | false |
| 31 | --enable-gravity | A boolean value that switches on (true) or off (false) the use of the gravity correction for the pressure gradients. | true |
| 32 | --enable-grid-adaptation | A boolean value that enables (true) or disables (false) adaptive grid refinement/coarsening. Note that Eclipse grid output will be disabled if adaptive grid refinement/coarsening is enabled. | false |
| 33 | --enable-intensive-quantity-cache | A boolean value that switches on (true) or off (false) the caching of intensive quantities. | true |
| 34 | --enable-logging-fallout-warning | A boolean value that sets the developer option to see whether logging was on non-root processors (true). If set to true output will be appended to the *.DBG or *.PRT files. | false |
...
| 55 | --linear-solver | A defined quoted character string that sets the configuration of the linear solver, valid values are: Option (2) extends the existing Constrained Pressure Residual (“CPR”) preconditioner to include wells. This option can also be invoked via the [CPR](#__RefHeading___Toc27871_3671211675) keyword in the [RUNSPEC](#__RefHeading___Toc55591_1778172979) section; however, the command line parameter takes precedence. For option (7) one enters a character string enclosed in quotes that defines the filename of a JSON configuration file for the flexible linear solver system. In this case the file extension should be ‘.json’. Note that the *.PRT file contains the “Property tree for the linear solver” listing, which is the JSON specification of the current case, and can be used to configure a user specific linear solver JSON file. The option "cpr" now is an alias for "cprw" instead of "cpr_trueimpes". | “ilu0” |
