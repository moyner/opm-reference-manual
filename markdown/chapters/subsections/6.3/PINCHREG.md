...
| 2 | PINCHOPT | A character string controlling the generation of pinch-outs when the MINPV keyword has been used to deactivate cells with small pore volumes. PINCHOPT can either be set to: | GAP |
...
| 5 | PINCHMUL | A character string controlling the calculation of the pinch-out transmissibilities when adjustments have been made by the MULTZ keyword.  PINCHMUL can either be set to: Note if PINCHCAL has been set equal to ALL then PINCHMUL is reset to TOP, irrespective of the entered value for PINCHMUL. | TOP |
| Notes: |  |  |  |
