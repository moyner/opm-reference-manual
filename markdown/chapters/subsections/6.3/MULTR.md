### MULTR – Multiply Cell Transmissibility in the +R Direction {#kw-MULTR}
...
MULTR multiples the transmissibility between two cell faces in the +R direction for all the cells in the model via an array, that is the keyword sets the transmissibility multiplier of block (I, J, K) between the cells (I, J, K) and (I+I, J, K).  An alternative to defining the complete array is to use the [BOX](#kw-BOX) keyword to define an area of the grid and then use the MULTR keyword to set the multipliers just for the area defined by the [BOX](#kw-BOX) keyword (see the example).
...
| Notes: |  |  |  |
: MULTR Keyword Description {#tbl-6-74}
See also the [MULTR-](#kw-MULTR-), [MULTTHT](#kw-MULTTHT), [MULTTHT-](#kw-MULTTHT-), [MULTZ](#kw-MULTZ) and [MULTZ-](#kw-MULTZ-) keywords for scaling transmissible between between grid cells in the R direction.
