### DIFFCGAS – Define PVT Region Gas Component Diffusion Coefficients {#kw-DIFFCGAS}
...
See also the [DIFFAGAS](#REF_HEADING_KEYWORD_DIFFAGAS_8_3) and [DIFFAWAT](#REF_HEADING_KEYWORD_DIFFAWAT_8_3) keywords that assume a mass fraction formulation for diffusion rather than the standard mole fraction formulation assumed by the DIFFCGAS and [DIFFCWAT](#kw-DIFFCWAT) keywords. The [DIFFAGAS](#REF_HEADING_KEYWORD_DIFFAGAS_8_3) and [DIFFAWAT](#REF_HEADING_KEYWORD_DIFFAWAT_8_3) keywords cannot be used in combination with the DIFFCGAS and [DIFFCWAT](#kw-DIFFCWAT) keywords.


::: {.callout-note}
This is an OPM Flow keyword used with OPM Flow’s [CO2STORE](#kw-CO2STORE) or [H2STORE](#REF_HEADING_KEYWORD_H2STORE) and [GASWAT](#kw-GASWAT) keywords in the [RUNSPEC](#kw-RUNSPEC) section, and should not be confused with the more general version of the DIFFCGAS keyword used in the commercial compositional simulator.
:::
...
| ft2/day | m2/day | cm2/hour |  |
| Notes: |  |  |  |
: DIFFCGAS Keyword Description {#tbl-8-28}
::: {.callout-note}
...
See also the [DIFFUSE](#kw-DIFFUSE) keyword in the [RUNSPEC](#kw-RUNSPEC) section to activate the Molecular Diffusion option and the [DIFFCWAT](#kw-DIFFCWAT) keyword in the [PROPS](#kw-PROPS) section that defines the water diffusion coefficients for each compositional component in the model and for each PVT region. Finally, for gas-oil systems the [DIFFC](#kw-DIFFC) keyword in the [PROPS](#kw-PROPS) section should be used.

Normally diffusion coefficients are measured in laboratory units, that is cm2/s, for ease of use, @tbl-8-29 outlines the conversion factors for converting the laboratory measured diffusion coefficients to those used by the simulator.
...
: Diffusivity Conversion Factors {#tbl-8-29}
#### Example
