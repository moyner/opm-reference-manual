...
| oF | oC | oC |  |
| 2 | VIS | A columnar vector of real increasing down the column values that defines the gas viscosity for the corresponding temperature values (TEMP). VIS should be given at the reference pressure defined by the PRESS variable on the VISCREF keyword. | None |
| cP | cP | cP |  |
| Notes: |  |  |  |
