### SSWCR – End-Point Scaling Grid Cell Critical Water Saturation {#kw-SSWCR}
...
SSWCR defines the surfactant critical water saturation for all the cells in the model via an array when the end-point scaling option has been invoked via the [ENDSCALE](#kw-ENDSCALE) keyword and the surfactant phase has been activated by the [SURFACT](#kw-SURFACT) keyword in the [RUNSPEC](#kw-RUNSPEC) section. The critical water saturation is defined as the maximum water saturation for which the water relative permeability is zero in a two-phase relative permeability table.  SSWCR scales the surfactant water relative permeability data.
...
The above example defines a constant surfactant critical water saturation of 0.20 to all 300 cells in the model as defined by the [DIMENS](#kw-DIMENS) keyword in the [RUNSPEC](#kw-RUNSPEC) section.