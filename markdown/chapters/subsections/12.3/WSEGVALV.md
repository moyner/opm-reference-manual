### WSEGVALV – Define Multi-Segment Well Sub-Critical Valve {#kw-WSEGVALV}
...
| 2 | ISEG1 | A positive integer greater than or equal to two and less than or equal to MXSEGS on the [WSEGDIMS](#kw-WSEGDIMS) keyword in the [RUNSPEC](#kw-RUNSPEC) section that defines the segment containing the sub-critical valve. | None |
...
| 10 | AREAMAX | A real positive value that defines the maximum cross-sectional area of flow in the restricted section of the valve (Amax). AREAMAX is used to convert the segment volumetric flow rate into the maximum flow velocity at the constriction (υr). If defaulted then AREAPIPE will be used if defined, otherwise XAREA (item 2-9) on the [WELSEGS](#kw-WELSEGS) keyword will be used. | Defined |
| ft2 | m2 | cm2 |  |
| Notes: |  |  |  |
: WSEGVALV Keyword Description {#tbl-12-121}
The total number of wells should be defined via the [WELLDIMS](#kw-WELLDIMS) keyword and the number of multi-segment wells should be declared on the [WSEGDIMS](#kw-WSEGDIMS) keyword, both keywords are in the [RUNSPEC](#kw-RUNSPEC) section. In addition, the [WELSPECS](#kw-WELSPECS) keyword should be used to define wells, the [COMPDAT](#kw-COMPDAT) keyword to define the well completions for both ordinary wells and multi-segment wells, and the [COMPSEGS](#kw-COMPSEGS) keyword to define a multi-segment segment completions. Finally, the WSEGVALV keyword can then be use to define ICD connections for the well.  All the aforementioned keywords are described in the [SCHEDULE](#kw-SCHEDULE) section.
...
C1 and C2	=	Unit conversion constants as listed in @tbl-12-122.
...
: WELVALV Conversion Factor Constants and Units {#tbl-12-122}
...
C2	=	Unit conversion constants as listed in @tbl-12-122.
...
The following example is based on one producing well segment oil well (OP01) using the [WELSPECS](#kw-WELSPECS), [WELSEGS](#kw-WELSEGS) [COMPDAT](#kw-COMPDAT) and [COMPSEGS](#kw-COMPSEGS) keywords, as per the [WSEGSICD](#kw-WSEGSICD) keyword example (Example), and is therefore not repeated here.
