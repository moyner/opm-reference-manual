### WSALT – Define Water Injection Well Salt Concentrations {#kw-WSALT}
...
The WSALT keyword defines a water injection well’s salt injection stream concentration that is to be used for when the salt option has been activated by the [BRINE](#kw-BRINE) keywords in the [RUNSPEC](#kw-RUNSPEC) section. Note that if the Polymer option has also been activated by the [POLYMER](#kw-POLYMER) keyword in the [RUNSPEC](#kw-RUNSPEC) section, then the [WPOLYMER](#kw-WPOLYMER) keyword in the [SCHEDULE](#kw-SCHEDULE) section should be used to enter both the polymer and salt concentrations.
...
| Notes: |  |  |  |
: WSALT Keyword Description {#tbl-12-117}
...
The following example defines the salt injection stream concentration for three water injection wells for when the brine phase has been activated by the [BRINE](#kw-BRINE) keyword in the [RUNSPEC](#kw-RUNSPEC) section.
...
The next example is based on using the Multi-Component Brine option, that is the [BRINE](#kw-BRINE) and [ECLMC](#kw-ECLMC) keywords have been used in the [RUNSPEC](#kw-RUNSPEC) section, and assuming three salts.
