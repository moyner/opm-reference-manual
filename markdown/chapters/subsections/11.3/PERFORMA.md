### PERFORMA – Export Standard Simulator Performance Summary Variables to File {#kw-PERFORMA}
...
The PERFORMA keyword activates the writing out of a standard set of OPM Flow simulation numerical performance summary variables to the [SUMMARY](#kw-SUMMARY) (*.SMSPEC and .*UNSMRY) and RSM (*.RSM) files. @tbl-11-31 lists the summary variables written out by the keyword.
...
| Notes: |  |  |
: Simulator Performance Summary Variables (Numerical Performance) {#tbl-11-31}
::: {.callout-note}
If the summary vector data is unavailable then zeros are written at each time step to the [SUMMARY](#kw-SUMMARY) and RSM file.
:::
