...
See also the RADIAL keyword in the RUNSPEC section (RADIAL – Radial Grid Activation Option). Both Spider and Radial grids created by OPM Flow will be displayed the same in OPM ResInsight, as they are both output as Irregular Corner-Point Grid files. The difference is that the radial model’s pore volumes are adjusted to match the radial geometry pore volumes, whereas, the Spider grid volumes are not adjusted. See the examples in the section on Radial Grids (Radial Grids) and Spider Grids (Spider Grids) for further information
