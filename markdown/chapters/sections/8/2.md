...
$$
{ρ}_{g} = \frac{m}{V} = \frac{P \mathit{[MW](#kw-MW)}}{\mathit{ZRT}}
$$ {#eq-8-3}
...
::: {.callout-note}
In oilfield units when we are dealing with oil reservoirs the gas formation volume factor is more commonly denoted as Bg and has the units of rb/scf. Whereas for gas reservoirs, the gas formation volume factor is Eg and has the units scf/rcf and is often described as the Gas Expansion Factor or GEF.
:::
...
$$
{ρ}_{o} = 144 x \mathit{fluid} \mathit{gradient} (\text{psi/ft})
$$ {#eq-8-8}
...
$$
{ρ}_{o} = 144 x 0.338 = 48.7 \frac{\mathit{lbm}}{{\mathit{ft}}^{3}}
$$ {#eq-8-9}
...
$$
{γ}_{o} = \frac{{ρ}_{o}}{1000 }
$$ {#eq-8-12}

Commonly relative density is quoted with reference to the reference phase, for example, γo = 0.780 (water = 1).

The American Petroleum Institute (“[API](#kw-API)”) classifies oils based on an [API](#kw-API) gravity (γ[API](#kw-API)), or degrees [API](#kw-API) (oAPI), the relationship between relative density (γo) of oil and [API](#kw-API) gravity (γ[API](#kw-API)) is given by:


$$
{γ}_{\mathit{[API](#kw-API)}} = \frac{141.5}{{γ}_{o}} - 131.5
$$ {#eq-8-13}
...
γ[API](#kw-API)	= [API](#kw-API) gravity (oAPI).


[API](#kw-API) density is commonly reported at the well site.
...
$$
{B}_{o} = \frac{{V}_{\mathit{rc}}}{{V}_{\mathit{sc}}}
$$ {#eq-8-14}
...
$$
{c}_{o} = - \frac{1}{{B}_{o}} \frac{{\mathit{dB}}_{o}}{\mathit{dP}}
$$ {#eq-8-16}
...
The following tables outline some general parameters used to classify reservoir fluids. @tbl-8-1 outlines some general producing characteristics used to define reservoir fluids.
...
| Usual [API](#kw-API) Gravity Range of Produced Fluid | 10 – 45+ | 40 – 50 | 45 – 65 | – |
...
: General Reservoir Fluid Classification {#tbl-8-1}
In terms of fluid composition reservoir fluids can be indicated by the values shown in @tbl-8-2 below.
...
: Reservoir Fluids Composition Indicators {#tbl-8-2}
In addition reservoir oil is further divided into Light, Medium, Heavy, Extra Heavy, and Bituman (Wilmon^[Wilmon, G. J. : “Economic Outlook for Extra-heavy Oils, Natural Bitumens and Shale Oils”, World Petroleum Congress (1987).]) as shown in @tbl-8-3 below:
...
| [RUNSPEC](#kw-RUNSPEC) Keywords | [OIL](#kw-OIL) | [OIL](#kw-OIL) | [GAS](#kw-GAS) | [GAS](#kw-GAS) | [GAS](#kw-GAS) | [GAS](#kw-GAS) | [WATER](#kw-WATER) | [WATER](#kw-WATER) |
|  | DISGAS |  | VAPOIL | None / VAPOIL |  |  |  |  |
|  |  |  |  | VAPWAT5 |  |  |  |  |
|  |  |  |  | WATER |  |  |  |  |
|  |  |  |  | BRINE |  |  | BRINE |  |
| Pressure Dependent PVT | [PVCDO](#kw-PVCDO) | [PVCO](#kw-PVCO) | [PVDG](#kw-PVDG) | [PVTG](#kw-PVTG) | [PVTWSALT](#kw-PVTWSALT) | [PVTSOL](#kw-PVTSOL) | [PVTW](#kw-PVTW) | [PVTWSALT](#kw-PVTWSALT) |
| [PVDO](#kw-PVDO) | [PVTO](#kw-PVTO) | [PVZG](#kw-PVZG) |  | PVTGW5 | [PVDS](#kw-PVDS) |  |  |  |
|  |  |  |  | PVTGWO5 |  |  |  |  |
| Miscellaneous | [RSCONST](#kw-RSCONST) |  | [RVCONST](#kw-RVCONST) |  | RWGSALT5 |  |  |  |
| [RSCONSTT](#kw-RSCONSTT) |  | [RVCONSTT](#kw-RVCONSTT) |  |  |  |  |  |  |
| Surface Density | [DENSITY](#kw-DENSITY) | [BDENSITY](#kw-BDENSITY) | [SDENSITY](#kw-SDENSITY) | [DENSITY](#kw-DENSITY) | [BDENSITY](#kw-BDENSITY) |  |  |  |
| [GRAVITY](#kw-GRAVITY) |  |  | [GRAVITY](#kw-GRAVITY) |  |  |  |  |  |


| Notes: |
| --- |
: Fluid Property Keywords versus Oil, Gas and Water Fluid Type {#tbl-8-4}
For the Dead Oil phases the [RSCONST](#kw-RSCONST) and [RSCONSTT](#kw-RSCONSTT) keywords are used to set a constant gas-oil ratio (Rs). In this case the Rs is independent of the reservoir pressure and Rs is also negligible, as in for example heavy oil type fluids.

Similarly for the Dry Gas phase, where the [RVCONST](#kw-RVCONST) and [RVCONSTT](#kw-RVCONSTT) keywords are used to set a condensate-gas ratio (Rv) which is independent of the reservoir pressure and is also negligible, as in for example dry gas type fluids.

For the Vaporized Water phase and model, both dry and wet gas can be incorporated. Note that the [PVTGW](#kw-PVTGW), [PVTGWO](#kw-PVTGWO), [RWGSALT](#kw-RWGSALT), and [VAPWAT](#kw-VAPWAT) keywords are OPM Flow specific keywords.

In addition for the Brine phase, then either the [SALT](#kw-SALT) or [SALTVD](#kw-SALTVD) keywords in the [SOLUTION](#kw-SOLUTION) section should be used to define the initial equilibration salt concentration for the model.

CO2 can either be used as Enhanced Oil Recovery (“EOR”) fluid by injecting the CO2 into an oil reservoir, or injected for CO2 storage. For the former, CO2 is declared via the [GAS](#kw-GAS) keyword in the [RUNSPEC](#kw-RUNSPEC) section and the PVT data is entered via the standard gas fluid properties for the hydrocarbon gas and either the [PVDS](#kw-PVDS) or the [PVTSOL](#kw-PVTSOL) keywords are used to describe the interaction of the in situ oil and the injected CO2.

In addition to the above the [ROCK](#kw-ROCK) keyword should be used to define the rock compressibility.

Similarly, @tbl-8-5 outlines the fluid property data keywords for the CO2 storage,  foam, polymer, solvent, and [MICP](#kw-MICP) phases. Note that for these phases multiple keywords can be used to define the desired property behavior.


| Fluid Property Keywords versus CO2 (Storage), Foam, Polymer, Solvent,  and [MICP](#kw-MICP) Fluid Types |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
| Fluid Type | CO2 (Storage) | Foam | Polymer | Solvent | MICP7 |
| [RUNSPEC](#kw-RUNSPEC) Keywords | [CO2STORE](#kw-CO2STORE) | [FOAM](#kw-FOAM) | [POLYMER](#kw-POLYMER) | [SOLVENT](#kw-SOLVENT) | [MICP](#kw-MICP) |
| [GAS](#kw-GAS) |  |  |  | [WATER](#kw-WATER) |  |
| [WATER](#kw-WATER) |  |  |  |  |  |
| Pressure Dependent PVT | Not required, calculated via correlations |  |  | [PVDS](#kw-PVDS) | [PVTW](#kw-PVTW) |
|  |  | PVTSOL6 |  |  |  |
| Surface Density |  |  |  | [SDENSITY](#kw-SDENSITY) | [DENSITY](#kw-DENSITY) |
| Miscellaneous | SALINITY5 | [FOAMADS](#kw-FOAMADS) | [PLMIXPAR](#kw-PLMIXPAR) |  | [BIOFPARA](#REF_HEADING_KEYWORD_BIOFPARA) |
|  | FOAMDCYO | PLYADS |  |  |  |
|  | FOAMDCYW | PLYADSS |  |  |  |
|  | FOAMFCN | PLYATEMP |  |  |  |
|  | FOAMFRM | PLYCAMAX |  |  |  |
...
|  |  | PLYVSCST |  |  |  |
|  |  | SKPRPOLY8 |  |  |  |
| Miscellaneous |  |  | SKPRWAT8 |  |  |
| Notes: |  |  |  |  |  |
: Fluid Property Keywords versus CO2 (Storage), Foam, Polymer, Solvent, and MICP Fluid Types {#tbl-8-5}
As mentioned previously, CO2 can either be used as an EOR fluid by injecting the gas into an oil reservoir, or injected for CO2 storage. For the latter, CO2 is declared via the [GAS](#kw-GAS) and [CO2STORE](#kw-CO2STORE) keywords in the [RUNSPEC](#kw-RUNSPEC) section, and, except for the [SALINITY](#kw-SALINITY) keyword, all the remaining PVT data is calculated automatically via correlations by the simulator. A full description of the underlying PVT models is described by Sandve at al.^[Sandve, T. H., Gasda, S. E., Rasmussen, A., and Rustad, A. B. Convective dissolution in field scale CO2 storage simulation using the OPM Flow simulator. Submitted to TCCS 11 – Trondheim Conference on CO2 Capture, Transport and Storage Trondheim, Norway – June 21-23, 2021.]

Typical live oil and dry gas PVT data from the Volve^[The Volve Data was approved for data sharing in 2018 by the initiative of the last Operating company, Equinor and approved by the license partners ExxonMobil E&P Norway AS and Bayerngas Norge AS in the end of 2017.] field is shown in Figure 8.7 and Figure 8.8, respectively. To fully define the live oil, dry gas, and water PVT properties for Volve the [DENSITY](#kw-DENSITY), [PVDG](#kw-PVDG), [PVTO](#kw-PVTO), and [PVTW](#kw-PVTW) keywords are employed.
...
| --- | --- | --- |
| Saturation | [SOWCR](#kw-SOWCR) | Critical oil-in-water saturation, that is the largest oil saturation for which the oil relative permeability is zero in an oil-water system. |
| [SWL](#kw-SWL) | Connate water saturation, that is the lowest water saturation in a water saturation function table. |  |
...
| [SWU](#kw-SWU) | Maximum water saturation in a water saturation table.  This is commonly set equal to one, unless there is a residual oil saturation below the defined OWC. |  |
| Relative Permeability | [KRO](#kw-KRO) | Relative permeability of oil at the maximum oil saturation. |
| [KRORW](#kw-KRORW) | Relative permeability of oil at the critical water saturation |  |
| [KRW](#kw-KRW) | Relative permeability of water at the maximum water saturation (normally the maximum water saturation is one). |  |
| [KRWR](#kw-KRWR) | Relative permeability of water at the residual oil saturation (or at the residual gas saturation in a gas-water run). |  |
| Capillary Pressure | [SWLPC](#kw-SWLPC) | Capillary pressure connate water saturation, that is the lowest water saturation in a water saturation function table to be scaled independently from the [SWL](#kw-SWL) relative permeability end-point value. |
: Oil-Water Relative Permeability End-Point Data Definitions {#tbl-8-6}
![Frame5](../../images/Frame5_be79ab2b1e5d.png)
![Image6](../../images/Image6_be79ab2b1e5d.png)

Similarly for gas-oil systems, Figure 8.10 illustrates a typical gas-oil relative permeability set of curves indicating the oil end-point data ([KRORG](#kw-KRORG) and (1 – [SOGCR](#kw-SOGCR))) and the gas end-point data ([KRGR](#kw-KRGR), [KRG](#kw-KRG), and [SGCR](#kw-SGCR)).
...
| --- | --- | --- |
| Saturation | [SOGCR](#kw-SOGCR) | Critical oil-in-gas saturation, that is the largest oil saturation for which the oil relative permeability is zero in an oil-gas-connate-water system. |
| [SGL](#kw-SGL) | Connate gas saturation, that is the lowest gas saturation in a gas saturation function table. |  |
| [SGCR](#kw-SGCR) | Critical gas saturation, that is the largest gas saturation for which the gas relative permeability is zero. |  |
| [SGU](#kw-SGU) | Maximum gas saturation in a gas saturation table. This is generally set equal to one minus the connate water saturation. |  |
...
| Capillary Pressure | [SGLPC](#kw-SGLPC) | Capillary pressure connate gas saturation, that is the smallest gas saturation in a gas saturation function table to be scaled independently from the [SGL](#kw-SGL) relative permeability end-point value. |
: Gas-Oil Relative Permeability End-Point Data Definitions {#tbl-8-7}
Finally, for two phase gas-water systems, Figure 8.11 illustrates a typical set of gas-water relative permeability curves indicating the gas end-point data ([KRG](#kw-KRG), [KRGR](#kw-KRGR), and (1 – SGWCR)) and the water end-point data ([KRW](#kw-KRW), [KRWR](#kw-KRWR), [SWL](#kw-SWL), and SWGCR).
...
| --- | --- | --- |
| Saturation | [SGL](#kw-SGL) | Connate gas saturation, that is the lowest gas saturation in a gas saturation function table. |
| [SGU](#kw-SGU) | Maximum gas saturation in a gas saturation table. |  |
...
| [SWU](#kw-SWU) | Maximum water saturation in a water saturation table. |  |
| Relative Permeability | [KRG](#kw-KRG) | Relative permeability of gas at the maximum gas saturation. |
| [KRGR](#kw-KRGR) | Relative permeability of gas at the residual oil saturation (or the critical water saturation in a gas-water run). |  |
...
| Capillary Pressure | [SWLPC](#kw-SWLPC) | Capillary pressure connate water saturation, that is the smallest water saturation in a water saturation function table to be scaled independently from the [SWL](#kw-SWL) relative permeability end-point value. |
: Gas-Water Relative Permeability End-Point Data Definitions {#tbl-8-8}
End-point scaling is activated in the [RUNSPEC](#kw-RUNSPEC) section with the [ENDSCALE](#kw-ENDSCALE) keyword and the data used to apply end-point scaling is entered in the [PROPS](#kw-PROPS) section using the end-point keywords defined in @tbl-8-6, @tbl-8-7, and @tbl-8-8 to define each grid block’s end-point data. There are also direction dependent versions of these keywords for when directional end-point scaling has been activated. For example for critical water saturation, [SWCR](#kw-SWCR) is used with non-directional end-point scaling and the SWCRX, SWCRX-, SWCRY, SWCRY-, SWCRZ, and SWCRZ- series of keywords is used for when directional end-point scaling has been activated. In addition, there is also the facility to incorporate end-point scaling based on the drainage and/or imbibition process which again can be either non-directional or directional.


::: {.callout-note}
If the hysteresis model option has been activated on the [SATOPTS](#kw-SATOPTS) keyword in the [RUNSPEC](#kw-RUNSPEC) section, then the standard end-point scaling arrays, for example [KRORW](#kw-KRORW), can be used to scale the drainage tables, and the equivalent imbibition arrays suffixed with the letter I, for example [IKRORW](#kw-IKRORW), can be used to scale the imbibition tables. However, if the hysteresis model option has not been activated, then only the standard keywords can be used to define the end-point scaling parameters, that is [KRORW](#kw-KRORW) should be used and not [IKRORW](#kw-IKRORW), even though an imbibition process with the wetting phase (normally water) increasing is usually being modelled.
:::
...
| [SGOF](#kw-SGOF) | Pcog |  | SGFN3 |  | Pcog |  |  |
| [SLGOF](#kw-SLGOF) | Pcog |  | [SGWFN](#kw-SGWFN) |  | Pcgw |  |  |
| [SWOF](#kw-SWOF) | Pcwo |  | Pcwo | SOF24 | No Pc |  |  |
|  |  |  |  | SOF35 | No Pc |  |  |
|  |  |  |  | SOF32D | No Pc |  |  |
|  |  |  |  | SWFN |  |  | Pcwo |
| OPM Flow LET6 | Format Type Three7 |  |  |  |  |  |  |
| Keyword | Oil | Gas | Water | Keyword | Oil | Gas | Water |
| [SGOFLET](#kw-SGOFLET) | Pcog |  | [GSF](#kw-GSF) |  | Pcgw |  |  |
| SWGFLET |  | Pcgw | [WSF](#kw-WSF) |  |  | No Pc |  |
| [SWOFLET](#kw-SWOFLET) | Pcwo |  | Pcwo |  |  |  |  |
| Notes: |  |  |  |  |  |  |  |
: Saturation Table Formats and Phases {#tbl-8-9}
...
As mentioned in the 2.4.1 Data Quality and Assurance section, Corey relative permeability curves are often used to "smooth" the intended relative permeability curves to be used in the simulator in order to improve convergence.
...
$$
{k}_{\mathit{rw}} ({S}_{w}) = {k}_{\mathit{rww}}{[\frac{{S}_{w} - {S}_{\mathit{wcr}}}{1 - {S}_{\mathit{orw}} - {S}_{\mathit{wcr}}}]}^{{n}_{w}}
$$ {#eq-8-20}
...
: Corey Exponent Values {#tbl-8-10}
...
: LET Modeling of Flooding Processes {#tbl-8-11}
...
$$
{K}_{\mathit{row}} = \frac{{K}_{\mathit{rot}}{(1 - {S}_{wp})}^{{L}_{o}}}{{(1 - {S}_{wp})}^{{L}_{o}} + {E}_{o}{{S}_{wp}}^{{T}_{o}}}
$$ {#eq-8-21}


$$
{K}_{\mathit{rw}} = \frac{{K}_{\mathit{rwt}}{S}_{wp}^{{L}_{w}}}{{S}_{wp}^{{L}_{w}} + {E}_{w}{(1 - {S}_{wp})}^{{T}_{w}}}
$$ {#eq-8-22}


$$
{S}_{wp} = \frac{{S}_{w} - {S}_{\mathit{wirr}}}{1 - {S}_{\mathit{wirr}}}
$$ {#eq-8-23}

where

${E}_{α}$	=	empirical E parameter for the α phase,

${K}_{\mathit{row}}$	= 	oil relative permeability with respect to water saturation,

${K}_{\mathit{rot}}$	= 	base or top oil relative permeability, used to scale the maximum oil

permeability,

${K}_{\mathit{rw}}$	=	water relative permeability,

${K}_{\mathit{rwt}}$	=	base or top water relative permeability, used to scale the maximum water

permeability,

${L}_{α}$	=	empirical L parameter for the α phase,

${T}_{α}$	=	empirical T parameter for the α phase,

${S}_{w}$	=	water saturation,

${S}_{\mathit{wirr}}$	=	irreducible water saturation, and

${S}_{wp}$	=	normalize water saturation.
...
$$
Y = \frac{{P}_{\mathit{cow}} - {P}_{\mathit{ct}}}{{P}_{\mathit{cir}} - {P}_{\mathit{ct}}} a\mathit{nd} Y = F({S}_{wp}) \mathit{where} {S}_{\mathit{wirr}}⩽{S}_{w}⩽1.0
$$ {#eq-8-24}


$$
{P}_{\mathit{cow}}({S}_{w}={S}_{\mathit{wirr}}) = {P}_{\mathit{cir}} \text{and} {P}_{\mathit{cow}}({S}_{w}=1.0) = {P}_{\mathit{cit}}
$$ {#eq-8-25}


$$
F = \frac{{(1 - {S}_{wp})}^{L}}{{(1 - {S}_{wp})}^{L} + E{S}_{wp}^{T}} \text{and} {S}_{wp} = \frac{{S}_{w} - {S}_{\mathit{wirr}}}{1 - {S}_{\mathit{wirr}}}
$$ {#eq-8-26}

where

$E$	= 	empirical E parameter,

$L$	=	empirical L parameter,

${P}_{\mathit{cir}}$	= 	capillary pressure at irreducible water saturation,

${P}_{\mathit{ct}}$	= 	capillary threshold / entry pressure,

$T$	=	empirical T parameter,
...
OPM has implemented a more restrictive form of the general LET functions, more akin to the original paper168, as depicted in Figure 8.20.  @tbl-8-12 relates the general LET equations presented previously with the parameters on the [SWOFLET](#kw-SWOFLET) keyword.


| LET Function (DIC1) versus [SWOFLET](#kw-SWOFLET) Keyword |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
| [SWOFLET](#kw-SWOFLET) | LET Function | [SWOFLET](#kw-SWOFLET) | LET Function | [SWOFLET](#kw-SWOFLET) | LET Function |
| [SWL](#kw-SWL) | ${S}_{\mathit{wirr}}$ | SORW |  | PCIR | ${P}_{\mathit{cir}}$ |
| [SWCR](#kw-SWCR) | ${S}_{\mathit{wirr}}$ | [SOWCR](#kw-SOWCR) |  | PCIT | ${P}_{\mathit{ct}}$ |
| LWAT | ${L}_{w}$ | LOIL | ${L}_{o}$ | LPC | $L$ |
| EWAT | ${E}_{w}$ | EOIL | ${E}_{o}$ | EPC | $E$ |
| TWAT | ${T}_{w}$ | TOIL | ${T}_{o}$ | TPC | $T$ |
| KRTWAT | ${K}_{\mathit{rwt}}$ | KRTOIL | ${K}_{\mathit{rot}}$ |  |  |
| Notes: |  |  |  |  |  |
:  LET Functions (DIC1) versus SWOFLET Keyword {#tbl-8-12}
...
$$
{K}_{\mathit{row}} = \frac{{K}_{\mathit{rot}}{(1 - {S}_{\mathit{wn}})}^{{L}_{o}}}{{(1 - {S}_{\mathit{wn}})}^{{L}_{o}} + {E}_{o}{{S}_{\mathit{wn}}}^{{T}_{o}}}
$$ {#eq-8-27}


$$
{K}_{\mathit{rw}} = \frac{{K}_{\mathit{rwr}}{S}_{\mathit{wn}}^{{L}_{w}}}{{S}_{\mathit{wn}}^{{L}_{w}} + {E}_{w}{(1 - {S}_{\mathit{wn}})}^{{T}_{w}}}
$$ {#eq-8-28}


$$
{S}_{\mathit{wn}} = \frac{{S}_{w} - {S}_{\mathit{wirr}}}{1 - {S}_{\mathit{orw}} - {S}_{\mathit{wirr}}}
$$ {#eq-8-29}
...
${K}_{\mathit{rwr}}$	=	base or top water relative permeability, used to scale the maximum water
...
${S}_{\mathit{orw}}$	= 	residual oil saturation under water displacement,

${S}_{w}$	=	water saturation,

${S}_{\mathit{wirr}}$	=	irreducible water saturation, and

${S}_{\mathit{wn}}$	=	normalize water saturation.
...
$$
\begin{matrix}{P}_{\mathit{cow}}=({P}_{\mathit{cir}} - {P}_{\mathit{ct}}){G}_{n} + ({P}_{\mathit{cor}} - {P}_{\mathit{ct}}){F}_{n} + {P}_{\mathit{ct}} \\  \\ \mathit{where} {S}_{\mathit{wir}}\leq {S}_{w}\leq (1-{S}_{\mathit{orw}}) \text{and} {P}_{\mathit{cow}}({S}_{w}={S}_{\mathit{wzo}}) = 0\text{ => }{E}_{\mathit{sn}}\end{matrix}
$$ {#eq-8-30}


$$
{G}_{n} = \frac{{(1 - {S}_{\mathit{wn}})}^{{L}_{\mathit{sn}}}}{{(1 - {S}_{\mathit{wn}})}^{{L}_{\mathit{sn}}} + {E}_{\mathit{sn}}{S}_{\mathit{wn}}^{{T}_{\mathit{sn}}}} {F}_{n} = \frac{{S}_{\mathit{wn}}^{{L}_{\mathit{fn}}}}{{S}_{\mathit{wn}}^{{L}_{\mathit{fn}}} + {E}_{\mathit{fn}}{(1 - {S}_{\mathit{wn}})}^{{T}_{\mathit{fn}}}}
$$ {#eq-8-31}
...
${E}_{\mathit{fn}}$	= 	forced imbibition empirical E parameter,

${E}_{\mathit{sn}}$	= 	spontaneous imbibition empirical E parameter,

${L}_{\mathit{fn}}$	=	forced imbibition empirical L parameter,

${L}_{\mathit{sn}}$	=	spontaneous imbibition empirical L parameter,

${P}_{\mathit{cir}}$	= 	capillary pressure at irreducible water saturation,

${P}_{\mathit{ct}}$	= 	capillary threshold / entry pressure,

${T}_{\mathit{fn}}$	=	forced imbibition empirical T parameter,

${T}_{\mathit{sn}}$	=	spontaneous imbibition empirical T parameter,
...
@tbl-8-13 relates the general LET equations presented previously with the parameters on the [SWOFLET](#kw-SWOFLET) keyword.


| LET Function (IDC2) versus [SWOFLET](#kw-SWOFLET) Keyword |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
| [SWOFLET](#kw-SWOFLET) | LET Function | [SWOFLET](#kw-SWOFLET) | LET Function | [SWOFLET](#kw-SWOFLET) | LET Function |
| [SWL](#kw-SWL) | ${S}_{\mathit{wirr}}$ | SORW | ${S}_{\mathit{orw}}$ | PCIR | ${P}_{\mathit{cir}}$ |
| [SWCR](#kw-SWCR) | ${S}_{\mathit{wirr}}$ | [SOWCR](#kw-SOWCR) | ${S}_{\mathit{orw}}$ | PCIT | ${P}_{\mathit{ct}}$ |
| LWAT | ${L}_{w}$ | LOIL | ${L}_{o}$ | LPC | ${L}_{\mathit{sn}}$ |
| EWAT | ${E}_{w}$ | EOIL | ${E}_{o}$ | EPC | ${E}_{\mathit{sn}}$ |
| TWAT | ${T}_{w}$ | TOIL | ${T}_{o}$ | TPC | ${T}_{\mathit{sn}}$ |
| KRTWAT | ${K}_{\mathit{rwr}}$ | KRTOIL | ${K}_{\mathit{rot}}$ |  |  |
| Notes: |  |  |  |  |  |
:  LET Functions (IDC2) versus SWOFLET Keyword {#tbl-8-13}
...
$$
{K}_{\mathit{rog}} = \frac{{K}_{\mathit{rot}}{(1 - {S}_{\mathit{gn}})}^{{L}_{o}}}{{(1 - {S}_{\mathit{gn}})}^{{L}_{o}} + {E}_{o}{{S}_{\mathit{gn}}}^{{T}_{o}}}
$$ {#eq-8-33}


$$
{K}_{\mathit{rg}} = \frac{{K}_{\mathit{rgt}}{S}_{\mathit{gn}}^{{L}_{g}}}{{S}_{\mathit{gn}}^{{L}_{g}} + {E}_{g}{(1 - {S}_{\mathit{gn}})}^{{T}_{g}}}
$$ {#eq-8-34}


$$
{S}_{\mathit{gn}} = \frac{{S}_{g}}{1 - {S}_{\mathit{org}} - {S}_{\mathit{wirr}}}
$$ {#eq-8-35}
...
${K}_{\mathit{rog}}$	=	oil relative permeability with respect to gas saturation,

${K}_{\mathit{rot}}$	=	base or top oil relative permeability, used to scale the maximum oil

permeability,

${K}_{\mathit{rg}}$	=	gas relative permeability,

${K}_{\mathit{rgt}}$	=	base or top gas relative permeability, used to scale the maximum gas
...
${S}_{g}$	=	gas saturation,

${S}_{\mathit{gn}}$	=	normalize gas saturation,

${S}_{\mathit{org}}$	= 	residual oil saturation under gas displacement, and

${S}_{\mathit{wirr}}$	=	irreducible water saturation.
...
$$
\begin{matrix}{P}_{\mathit{cog}}=({P}_{\mathit{cir}} - {P}_{\mathit{ct}}){G}_{n} + ({P}_{\mathit{cor}} - {P}_{\mathit{ct}}){F}_{n} + {P}_{\mathit{ct}} \\  \\ \mathit{where}{S}_{\mathit{wir}}\leq {S}_{w}\leq (1-{S}_{\mathit{org}}-{S}_{\mathit{wirr}}) \text{and} {P}_{\mathit{cog}}({S}_{w}={S}_{\mathit{wzo}}) = 0\text{ => }{E}_{\mathit{sn}}\end{matrix}
$$ {#eq-8-36}


$$
{G}_{n} = \frac{{(1 - {S}_{\mathit{gn}})}^{{L}_{\mathit{sn}}}}{{(1 - {S}_{\mathit{gn}})}^{{L}_{\mathit{sn}}} + {E}_{\mathit{sn}}{S}_{\mathit{gn}}^{{T}_{\mathit{sn}}}} {F}_{n} = \frac{{S}_{\mathit{gn}}^{{L}_{\mathit{fn}}}}{{S}_{\mathit{gn}}^{{L}_{\mathit{fn}}} + {E}_{\mathit{fn}}{(1 - {S}_{\mathit{gn}})}^{{T}_{\mathit{fn}}}}
$$ {#eq-8-37}
...
@tbl-8-14 relates the general LET equations presented previously with the parameters on the [SGOFLET](#kw-SGOFLET) keyword.


| LET Function (CDI2) versus [SGOFLET](#kw-SGOFLET) Keyword |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
| [SGOFLET](#kw-SGOFLET) | LET Function | [SGOFLET](#kw-SGOFLET) | LET Function | [SGOFLET](#kw-SGOFLET) | LET Function |
| [SGL](#kw-SGL) |  | SORG | ${S}_{\mathit{org}}$ | PCIR | ${P}_{\mathit{cir}}$ |
| [SGCR](#kw-SGCR) |  | [SOGCR](#kw-SOGCR) | ${S}_{\mathit{org}}$ | PCIT | ${P}_{\mathit{ct}}$ |
| LGAS | ${L}_{g}$ | LOIL | ${L}_{o}$ | LPC | ${L}_{\mathit{sn}}$ |
| EGAS | ${E}_{g}$ | EOIL | ${E}_{o}$ | EPC | ${E}_{\mathit{sn}}$ |
| TGAS | ${T}_{g}$ | TOIL | ${T}_{o}$ | TPC | ${T}_{\mathit{sn}}$ |
| KRTGAS | ${K}_{\mathit{rgt}}$ | KRTOIL | ${K}_{\mathit{rot}}$ |  |  |
| Notes: |  |  |  |  |  |
:  LET Functions (CDI2) versus SGOFLET Keyword {#tbl-8-14}
...
$$
{K}_{\mathit{rgw}} = \frac{{K}_{\mathit{rgt}}{(1 - {S}_{wp})}^{{L}_{g}}}{{(1 - {S}_{wp})}^{{L}_{g}} + {E}_{g}{{S}_{wp}}^{{T}_{g}}}
$$ {#eq-8-39}
...
${K}_{\mathit{rgw}}$	= 	gas relative permeability with respect to water saturation,
...
$$
Y = \frac{{P}_{\mathit{cgw}} - {P}_{\mathit{ct}}}{{P}_{\mathit{cir}} - {P}_{\mathit{ct}}} a\mathit{nd} Y = F({S}_{wp}) \mathit{where} {S}_{\mathit{wirr}}⩽{S}_{w}⩽1.0
$$ {#eq-8-42}


$$
{P}_{\mathit{cgw}}({S}_{w}={S}_{\mathit{wirr}}) = {P}_{\mathit{cir}} \text{and} {P}_{\mathit{cgw}}({S}_{w}=1.0) = {P}_{\mathit{cit}}
$$ {#eq-8-43}
...
@tbl-8-15 relates the general LET equations presented previously with the parameters on the [SGWFLET](__RefHeading___Toc548127_947687768) keyword.


| LET Function (DCI1) versus [SGWFLET](#kw-SGWFLET) Keyword |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
| [SGWFLET](#kw-SGWFLET) | LET Function | [SGWFLET](#kw-SGWFLET) | LET Function | [SGWFLET](#kw-SGWFLET) | LET Function |
| [SGL](#kw-SGL) |  | [SWL](#kw-SWL) | ${S}_{\mathit{wirr}}$ | PCIR | ${P}_{\mathit{cir}}$ |
| [SGCR](#kw-SGCR) |  | [SWCR](#kw-SWCR) | ${S}_{\mathit{wirr}}$ | PCIT | ${P}_{\mathit{ct}}$ |
| LGAS | ${L}_{g}$ | LWAT | ${L}_{w}$ | LPC | $L$ |
| EGAS | ${E}_{g}$ | EWAT | ${E}_{w}$ | EPC | $E$ |
| TGAS | ${T}_{g}$ | TWAT | ${T}_{w}$ | TPC | $T$ |
| KRTGAS | ${K}_{\mathit{rgt}}$ | KRTWAT | ${K}_{\mathit{rwr}}$ |  |  |
| Notes: |  |  |  |  |  |
:  LET Functions (DCI1) versus SGWFLET Keyword {#tbl-8-15}
::: {.callout-note}
The [SGWFLET](#kw-SGWFLET) keyword has not been implemented, as of this current release.
:::
...
$$
{K}_{\mathit{rgw}} = \frac{{K}_{\mathit{rgt}}{(1 - {S}_{\mathit{wn}})}^{{L}_{o}}}{{(1 - {S}_{\mathit{wn}})}^{{L}_{o}} + {E}_{o}{{S}_{\mathit{wn}}}^{{T}_{o}}}
$$ {#eq-8-45}
...
$$
{S}_{\mathit{wn}} = \frac{{S}_{w} - {S}_{\mathit{wirr}}}{1 - {S}_{\mathit{grw}} - {S}_{\mathit{wirr}}}
$$ {#eq-8-47}
...
${K}_{\mathit{rot}}$	=	base or top gas relative permeability, used to scale the maximum gas
...
${S}_{\mathit{ogw}}$	= 	residual gas saturation under water displacement,
...
$$
\begin{matrix}{P}_{\mathit{cgw}}=({P}_{\mathit{cir}} - {P}_{\mathit{ct}}){G}_{n} + ({P}_{\mathit{cgr}} - {P}_{\mathit{ct}}){F}_{n} + {P}_{\mathit{ct}} \\  \\ \mathit{where} {S}_{\mathit{wir}}\leq {S}_{w}\leq (1-{S}_{\mathit{gorw}}) \text{and} {P}_{\mathit{cgw}}({S}_{w}={S}_{\mathit{wzo}}) = 0\text{ => }{E}_{\mathit{sn}}\end{matrix}
$$ {#eq-8-48}
...
@tbl-8-16 relates the general LET equations presented previously with the parameters on the [SGWFLET](__RefHeading___Toc548127_947687768) keyword.
...
| [SGL](#kw-SGL) |  | [SWL](#kw-SWL) | ${S}_{\mathit{wirr}}$ | PCIR | ${P}_{\mathit{cir}}$ |
| [SGCR](#kw-SGCR) |  | [SWCR](#kw-SWCR) | ${S}_{\mathit{wirr}}$ | PCIT | ${P}_{\mathit{ct}}$ |
| LGAS | ${L}_{g}$ | LWAT | ${L}_{w}$ | LPC | ${L}_{\mathit{sn}}$ |
| EGAS | ${E}_{g}$ | EWAT | ${E}_{w}$ | EPC | ${E}_{\mathit{sn}}$ |
| TGAS | ${T}_{g}$ | TWAT | ${T}_{w}$ | TPC | ${T}_{\mathit{sn}}$ |
| KRTGAS | ${K}_{\mathit{rgt}}$ | KRTWAT | ${K}_{\mathit{rwr}}$ |  |  |
| Notes: |  |  |  |  |  |
:  LET Functions (DCI1) versus SGWFLET Keyword {#tbl-8-16}
::: {.callout-note}
The [SGWFLET](#kw-SGWFLET) keyword has not been implemented, as of this current release.
:::
...
If none of the relative permeability assignment options have been activated on the [SATOPTS](#kw-SATOPTS) keyword in the [RUNSPEC](#kw-RUNSPEC) section then saturation tables may be alloacted to each cell using the [SATNUM](#kw-SATNUM) keyword in the [REGIONS](#kw-REGIONS) section.

If the directional relative permeability option has been activated by the DIRECT parameter on the [SATOPTS](#kw-SATOPTS) keyword then direction dependent relative permeability tables may be allocated to each cell using the KRNUMX, KRNUMY, and KRNUMZ series of keywords in the [REGIONS](#kw-REGIONS) section. If the irreversible directional relative permeability option has also been activated by the DIRECT and IRREVERS parameters on the [SATOPTS](#kw-SATOPTS) keyword then relative permeability tables may be allocated to each face of each cell using the KRNUMX, KRNUMX-, KRNUMY, KRNUMY-, KRNUMZ, and KRNUMZ- series of keywords.

If the hysteresis option has been activated by the HYSTER parameter on the [SATOPTS](#kw-SATOPTS) keyword then the "drainage" and imbibition saturation function tables may be allocated to each cell using the [SATNUM](#kw-SATNUM) and [IMBNUM](#kw-IMBNUM) keywords respectively in the [REGIONS](#kw-REGIONS) section.

If both the hystersis and directional relative permeability options have been activated then the "drainage" relative permeabilities can be allocated using the KRNUMX, KRNUMY, and KRNUMZ series of keywords, and the imbibition relative permeabilities can be allocated using the IMBNUMX, IMBNUMY, and IMBNUMZ series of keywords.

If the hystersis, directional, and irreversible relative permeability options have been activated then the "drainage" relative permeabilities can be allocated using the KRNUMX, KRNUMX-, KRNUMY, KRNUMY-, KRNUMZ, and KRNUMZ- series of keywords, and the imbibition relative permeabilities can be allocated using the IMBNUMX, IMBNUMX-, IMBNUMY, IMBNUMY-, IMBNUMZ, and IMBNUMZ- series of keywords.

The above discussion assumes that the model has a Cartesian grid; however, if the model has a radial grid then the equivalent radial saturation table allocation keywords should be used. Thus, the equivalent [KRNUM](#kw-KRNUM) keywords would be KRNUMR, KRNUMR-, KRNUMT, KRNUMT-, KRNUMZ, and KRNUMZ-, and the equivalent [IMBNUM](#kw-IMBNUM) keywords would be IMBNUMR, IMBNUMR-, IMBNUMT, IMBNUMT-, IMBNUMZ, and IMBNUMZ-. Note that the KRNUMZ, KRNUMZ-, IMBNUMZ, and IMBNUMZ- keywords are the same for both Cartesian and radial grids.
