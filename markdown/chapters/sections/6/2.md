...
::: {.callout-note}
Radial grids are currently not fully implemented in OPM Flow as the completion of the circle in the THETA direction is not supported. For reference only, this feature is activated via the COMPLETE parameter on the COORDSYS keyword in the GRID section.
:::
...
$$
\mathit{PV} = \mathit{Cell}\mathit{Gross}\mathit{Volume}\times \mathit{PORO}\times \mathit{NTG}\times \mathit{ACTNUM}
$$ {#eq-6-1}
...
$$
{T}_{{x}_{i+½, j}} = {\left[\frac{{k}_{x} h (\mathit{Δy})}{μ (\mathit{Δx})}\right] }_{i+½, j}
$$ {#eq-6-2}
